  ...
  useEffect(() => {
    const fetchRentedProducts = async () => {
      try {
        ...
        const rentedResponse = await axios.get(
          `${import.meta.env.VITE_BACKEND_BASE_URL}/api/rented-products`,
          {
            headers: {
        ...
        const productPromises = rentedResponse.data.map(
          async (rentedProduct) => {
            const productResponse = await axios.get(
              `${import.meta.env.VITE_BACKEND_BASE_URL}/api/products/${
            ...
            <Col md={4} key={rentedProduct._id} className="mb-4">
              <Card>
                <Card.Img
                  variant="top"
                  src={
                    (rentedProduct.product.images &&
                      rentedProduct.product.images[0]) ||
                ...
                <Card.Body>
                  <Card.Title>
                    {rentedProduct.product.name || "Product Name"}
                  </Card.Title>
                  <Card.Text>
                    {rentedProduct.product.description ||
                      "No description available."}
                  </Card.Text>
                  <Card.Text>
                    <strong>
                      Price: ₹
                      {rentedProduct.product.price
                        ? rentedProduct.product.price.toFixed(2)
                        : "0.00"}
                    </strong>
                  </Card.Text>
                  <Card.Text>
                    <small className="text-muted">
                      Location:{" "}
                      {rentedProduct.product.location || "Not specified"}
                    </small>
                  </Card.Text>
                  <Card.Text>
                    <small className="text-muted">
                      Rented At:{" "}
                      {new Date(rentedProduct.rentedAt).toLocaleDateString()}
                    </small>
                  </Card.Text>
                </Card.Body>
              </Card>
